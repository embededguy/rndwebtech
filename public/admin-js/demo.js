...
window.onload = () => {
    ...
    modules: {
      ...
      'better-table': {
        operationMenu: {
          ...
          color: {
            colors: ['red', 'green', 'yellow', 'white', 'blue', 'black']
          }
        }
      },
      keyboard: {
  ...
  if(cont){
	  document.getElementsByClassName('ql-editor')[0].innerHTML = cont;
  	
  }
  quill.on('text-change', function() {
	    var quillData = quill.root.innerHTML;
	    document.getElementById('content-html').value = quillData;
	});
  
  let tableModule = quill.getModule('better-table');

  document.body.querySelector('#insert-table').onclick = (e) => {
  	e.preventDefault()
    tableModule.insertTable(parseInt(document.getElementById('prow').value), parseInt(document.getElementById('pcolumn').value));
  };
};
